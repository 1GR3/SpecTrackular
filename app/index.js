...
let FitBitHuez = ['#F83C40', '#FC6B3A', '#FFFF00', '#B8FC68', '#00A629', '#5BE37D', '#3BF7DE', '#14D3F5', '#3182DE', '#BD4EFC', '#F80070', '#F83478'];
...
function updateClock() {
	...
	if (today.adjusted.activeZoneMinutes && today.adjusted.activeZoneMinutes.total !== undefined) {
		acLabel.text = today.adjusted.activeZoneMinutes.total;
	} else {
		acLabel.text = 0;
	}
...
const clearBigDisplay = function() {
	bigText.text = '';
	bigDisplay.style.opacity = 0;
};
...
timeBtn.onmousedown = function() {
	...
	bigText.text = stLabel.text = hours + ':' + formattedMinutes;
	bigDisplay.style.opacity = 1;
	displayTimeout = setTimeout(function() {
		clearBigDisplay();
	}, 5000);
